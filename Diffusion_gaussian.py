...
from noise_gaussian import reverse_transform_pil, reverse_transform_tensor, sample_q
...
model_saved_file_path = Path('./checkpoints/saved_model_gaussian.pth')
...
for epoch in range(start_epoch, epochs):
  for batch_index, batch in enumerate(train_dataloader):
    ...
    if current_step % sample_every == 0:
      ...
      save_image(sample_images, str(results_folder / f'sample_gaussian_{current_step}.png'), nrow=batch_to_sample)

  saved_model['epoch'] = epoch + 1
  saved_model['model'] = model.state_dict()
  saved_model['optimizer']= optimizer.state_dict()
  torch.save(saved_model, './checkpoints/saved_model_gaussian.pth')
