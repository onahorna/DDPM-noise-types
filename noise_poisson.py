...
def sample_q(x0, t, noise=None):
    if noise is None:
        # Poisson noise application instead of Gaussian noise
        image = torch.clamp((x0 + 1) / 2, 0, 1)
        noise = torch.poisson(image * 255) / 255
        noise = noise * 2 - 1  # To scale the noise to [-1, 1] range
...
show_noisy_images(noisy_images, noise_type='poisson', original_image_name=os.path.basename(image_path))
