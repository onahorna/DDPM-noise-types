...
# Precompute the values of betas and alphas
num_timesteps = 1000
...
# Reverse Process Sampling with Noise Type Handling
@torch.no_grad()
def sample_p(model, x_t, t, noise_type='gaussian', clipping=True):
    """
    Sample from p_θ(xₜ₋₁ | xₜ) to get xₜ₋₁ according to Algorithm 2 with specified noise types.
    """
    ...
    # Handling the noise based on the noise type
    if noise_type == 'gaussian_blur':
        noise = torch.randn_like(x_t)  # For Gaussian blur, assume Gaussian noise is involved in forward process

    elif noise_type == 'gaussian':
        noise = torch.randn_like(x_t)  # Standard Gaussian noise for other cases

    elif noise_type == 'speckle':
        noise = torch.randn_like(x_t) * x_t  # Speckle noise: multiplicative noise (just an example)

    elif noise_type == 'salt_and_pepper':
        noise = (torch.rand_like(x_t) > 0.95).float() * torch.randint_like(x_t, 0, 2)  # Salt and pepper noise simulation

    # Clipping and adjusting the reverse process (model-based denoising)
    if clipping:
...
@torch.no_grad()
def sampling(model, shape, noise_type='gaussian', image_noise_steps_to_keep=1):
    """
    Implementing Algorithm 2 - reverse sampling with specified noise type.
    Args:
        model (torch.Module): The model that predicts the noise
        shape (tuple): Shape of the data (batch, channels, image_size, image_size)
        noise_type (str): Type of noise used ('gaussian', 'gaussian_blur', 'speckle', 'salt_and_pepper')
    Returns:
        list: List containing the images at different steps of the reverse process
    """
    batch = shape[0]
    images = torch.randn(shape, device=device)  # Pure noise as the starting point
    images_list = []
