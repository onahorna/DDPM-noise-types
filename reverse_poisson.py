...
# Reverse Process Sampling for Poisson Noise
@torch.no_grad()
def sample_p(model, x_t, t, noise_type='poisson', clipping=True):
    """
    Sample from p_θ(xₜ₋₁|xₜ) to get xₜ₋₁ according to Algorithm 2.
    Adapted for Poisson noise handling.
    """
    ...
    if noise_type == 'poisson':
        # Poisson denoising adjustment (approximating the noise inverse)
        predicted_mean = predicted_mean * (1 - torch.exp(-predicted_mean))  # Approximation for Poisson noise removal.
...
@torch.no_grad()
def sampling(model, shape, image_noise_steps_to_keep=1, noise_type='poisson'):
    """
    Implementing Algorithm 2 - reverse sampling.
    Args:
    ...
    """
    batch = shape[0]
    images = torch.randn(shape, device=device)  # Start with pure noise
    images_list = []
