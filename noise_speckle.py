...
# Modify sample_q function to add speckle noise
def sample_q(x0, t, noise=None):
    if noise is None:
        # Generating multiplicative (speckle) noise
    ...
    x_t = sqrt_alphas_bar_t_sampled * x0 * (1 + sqrt_1_minus_alphas_bar_t_sampled * noise)
    return x_t
...
# Generate and save noisy images with speckle noise
noisy_images = [[get_noisy_image(x0, torch.tensor([t])) for t in [0, 50, 100, 150, 200]]]
show_noisy_images(noisy_images, noise_type='speckle',  original_image_name=os.path.basename(image_path))
