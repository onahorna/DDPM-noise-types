...
# Precalculated values of betas and alphas
num_timesteps = 1000
...
# Posterior variance as defined in the forward process
...
def sample_p(model, x_t, t, clipping=True):
    """
    Sample from p_θ(xₜ₋₁|xₜ) to get xₜ₋₁ according to the reverse process.
    """
    ...
    if clipping:
        ...
        # Denosing and predicting the mean
...
def sampling(model, shape, image_noise_steps_to_keep=1):
    ...
    batch = shape[0]
    images = torch.randn(shape, device=device)  # Pure noise (start with random noise)
    images_list = []
