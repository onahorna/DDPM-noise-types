...
def salt_and_pepper_noise(image, salt_prob=0.01, pepper_prob=0.01):
    ...
    image[salt_mask] = 1.0  # Set salt pixels to white

    # Pepper noise (black pixels)
    pepper_mask = noise > (1 - pepper_prob)
    image[pepper_mask] = 0.0  # Set pepper pixels to black

    return image

def sample_q(x0, t, noise=None):
    if noise is None:
        noise = torch.randn_like(x0)  # Default to Gaussian noise

    # Apply salt and pepper noise instead of Gaussian noise
    noisy_image = salt_and_pepper_noise(x0.clone(), salt_prob=0.05, pepper_prob=0.05)
    ...
    x_t = sqrt_alphas_bar_t_sampled * x0 + sqrt_1_minus_alphas_bar_t_sampled * noisy_image
    return x_t
...
show_noisy_images(noisy_images, noise_type='salt_and_pepper',  original_image_name=os.path.basename(image_path))
