...
from torchvision.transforms import ToPILImage, Compose, ToTensor, ColorJitter, Resize, Normalize
...
def get_noisy_image(x0, t, transform=reverse_transform_pil):
    # Apply ColorJitter transformation to simulate color noise
    color_jitter = ColorJitter(brightness=0.5, hue=0.3)
    jittered_image = color_jitter(reverse_transform_pil(x0.squeeze()))  # Apply ColorJitter to the PIL image
...
# Generate and save noisy images with ColorJitter
noisy_images = [[get_noisy_image(x0, torch.tensor([t])) for t in [0, 50, 100, 150, 200]]]
show_noisy_images(noisy_images, noise_type='colorjitter', original_image_name=os.path.basename(image_path))
