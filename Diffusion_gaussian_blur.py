...
from noise_gaussian_blur import reverse_transform_pil, reverse_transform_tensor, sample_q
from unet_model import DiffusionUnet
from reverse_gaussian_blur import sample_p, sampling
import numpy as np
...
model_saved_file_path = Path('/checkpoints/saved_model_gaussian_blur.pth')
...
for epoch in range(start_epoch, epochs):
  for batch_index, batch in enumerate(train_dataloader):
    ...
    if current_step % sample_every == 0:
      ...
      save_image(sample_images, str(results_folder / f'sample_gaussian_blur_{current_step}.png'), nrow=batch_to_sample)

  saved_model['epoch'] = epoch + 1
  saved_model['model'] = model.state_dict()
  saved_model['optimizer']= optimizer.state_dict()
  torch.save(saved_model, '/checkpoints/saved_model_gaussian_blur.pth')
